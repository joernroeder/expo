...
class ContactRecordDomainMapper {
  ...
  fun toInsertable(record: NewRecord): Insertable {
    return when (record) {
      is EmailRecord.New -> EmailMapper.toNew(record)
      is PhoneRecord.New -> PhoneMapper.toNew(record)
      is DateRecord.New -> EventMapper.toNew(record)
      is ExtraNameRecord.New -> NicknameMapper.toNew(record)
      is PostalAddressRecord.New -> StructuredPostalMapper.toNew(record)
      is RelationshipRecord.New -> RelationMapper.toNew(record)
      is UrlAddressRecord.New -> WebsiteMapper.toNew(record)
  ...
  fun toDomain(record: CreateContactRecord): NewContact {
    val insertables = mutableListOf<Insertable>().apply {
  ...
  fun toPatchContact(
    ...
    } else if (record.isChangingStructuredName()) {
    ...
    } else if (record.isChangingOrganization()) {
