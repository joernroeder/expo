...
import expo.modules.contacts.next.mappers.isChangingOrganization
import expo.modules.contacts.next.mappers.isChangingStructuredName
...
class Contact(
  ...
  suspend fun getDetails(fields: Set<ContactField>?): GetContactDetailsRecord {
    ...
    return mapper.toRecord(repository.getContactWithDetails(extractableFields, contactId))
  }

  suspend fun delete() = repository.delete(contactId)

  suspend fun patch(patchContactRecord: PatchContactRecord): Boolean {
    val isChangingStructuredName = patchContactRecord.isChangingStructuredName()
    val isChangingOrganization = patchContactRecord.isChangingOrganization()

    var structuredNameDataId: DataId? = null
    var organizationDataId: DataId? = null

    if (isChangingStructuredName || isChangingOrganization) {
      val fieldsToFetch = mutableSetOf<ExtractableField<*>>()
      if (isChangingStructuredName) {
        fieldsToFetch.add(StructuredNameField)
      }
      if (isChangingOrganization) {
        fieldsToFetch.add(OrganizationField)
      }
      val existingContact = repository.getContactWithDetails(
        fieldsToFetch,
        contactId
      )
      if (isChangingStructuredName) {
    ...
    val rawContactId = repository.getRawContactId(contactId)
      ?: throw Exception("Could not find a rawContactId, It's because local contact does not exist, create new with Contact.create()")
  ...
  suspend fun editWithForm() = withContext(Dispatchers.IO) {
    ...
    val lookupUri = ContactsContract.Contacts.getLookupUri(contactId.value.toLong(), lookupKey)
    intentDelegate.launchEditContact(lookupUri)
  }
  ...
  companion object {
    ...
    suspend fun pick(
      ...
      return contactIntentDelegate.launchPickContact()?.let { uri ->
        val contactIdString = uri.lastPathSegment ?: throw UnableToExtractIdFromUriException(uri)
