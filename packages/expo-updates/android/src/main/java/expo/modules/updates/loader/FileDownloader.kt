...
class FileDownloader(
  ...
  private fun parsePatchResponseMetadata(response: Response): PatchResponseMetadata? {
    ...
    if (!hasImBsdiff && statusCode != 226 && !hasLegacyContentType) {
      return null
    }
  ...
  private fun validatePatchResponseMetadata(
    ...
    val actualBase = deltaBase ?: expoBaseUpdateId

    if ((!metadata.hasImBsdiff && metadata.statusCode != 226 && !metadata.hasLegacyContentType) || actualBase == null) {
      return false
    }

    if (deltaBase != null && expoBaseUpdateId != null && deltaBase != expoBaseUpdateId) {
      return false
    }

    if (expectedBase != null && actualBase != expectedBase) {
      return false
    }

    if (expectedTarget != null && eTag != null && eTag != expectedTarget) {
      return false
    }
